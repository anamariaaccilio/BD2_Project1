...
- Al momento de insertar o eliminar una cantidad pequeña de registros, el Extendable Hashing es competente. Sin embargo, cuando utilizamos una mayor cantidad de registros, la estructura más eficiente es el AVLFile gracias a su tiempo logarítmico. (específicamente a esta tarea). Sin embargo, la reestructuración en el AVL puede ser muy costosa. Con respecto al Sequential File, en promedio la eficiencia temporal es muy costosa con respecto a las otras estructuras. Los tiempos de ejecución son variables, cosa que no pasa con el Extendable ni el AVL.
- Con respecto a la eficiencia de búsqueda, el Sequential File tiene tiempos de búsqueda lineales, ya que los registros se almacenan en orden secuencial. El AVLFile, por otro lado, ofrece tiempos de búsqueda más eficientes en comparación, ya que permite búsquedas en tiempo logarítmico, lo que es considerablemente más rápido que una búsqueda secuencial. Sin extenderse más, el Extendable Hash ofrece tiempos de búsqueda cercanos a constante en promedio, ya que utiliza una función hash para buscar registros en buckets. Esto es especialmente eficiente para conjuntos de datos grandes, ya que el rendimiento se mantiene constante.
- Con respecto al uso de memoria, el Sequential File tiende a utilizar menos memoria, ya que los registros se almacenan en orden secuencial y no requieren estructuras adicionales como árboles o tablas de dispersión. El AVLFile requiere memoria adicional para mantener la estructura del árbol AVL, lo que puede aumentar el uso de memoria, especialmente para conjuntos de datos grandes. Por último, el Extendable Hash también requiere memoria adicional para mantener el índice y las estructuras de buckets, lo que puede aumentar el uso de memoria, pero suele ser más eficiente que AVLFile en términos de espacio.
- Con respecto a la escalabilidad, el Sequential File puede enfrentar problemas de escalabilidad cuando se trata de conjuntos de datos muy grandes debido a la búsqueda secuencial. Por otro lado, el AVLFile tiene una mejor escalabilidad que el Sequential File debido a su estructura de árbol balanceado, pero aún puede encontrar dificultades con conjuntos de datos extremadamente grandes (por temas de memoria). Finalmente, el Extendable Hash es el que ofrece una buena escalabilidad debido a la función hash y la capacidad de dividir buckets cuando es necesario. Puede manejar eficazmente grandes conjuntos de datos en este aspecto.
